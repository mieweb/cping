// $Id: cping_common.c,v 1.6 2007/08/14 04:31:09 jasone Exp $

#include "cping_common.h"
#include <ctype.h>
#include <sched.h>
...
// DRH---  NOT THREAD SAFE!
	static int init=0;
	static struct timeval  tv1, tv2, *tvold, *tvnew,*tmp;
   ...
   char     *stime;
	...
	gettimeofday( tvnew, &tz);
	ret = (tvnew->tv_sec - tvold->tv_sec) * 1000000 + (tvnew->tv_usec - tvold->tv_usec);
...
void common_options(int ch)
{
   failed_timeout=0;
	...
	case 'F':				// Jason E
	   ...
	   failed_timeout = atoi(optarg) * 1000;	// timing is in nanoseconds? 
	   failed_only = 1;
	   break;
...
restamp:
	...
	if (options & F_FLOOD) {
		...
		int i;
		__u8 *cp, *dp;
		if (!failed_only || triptime > failed_timeout) {							// Added by Jason E
		...
		if (failed_only && triptime > failed_timeout) { 							// Added by Jason E
		   ...
		   printf(" -- TO > thresh %ld ms %20.20s",failed_timeout/1000,stime);
		}

		if (failed_only)														// Added by Jason E
		   printf("\n");
		}
		if (dupflag)
			printf(" (DUP!)");
